...
import {
  ...
  Activity, User, Wallet, Scale, Menu
} from 'lucide-react';
...
function SidebarContent({ links, location, displayName, displayInitial, displayRole, logout, isMobile = false }: any) {
  return (
    <>
      {/* Subtle glow */}
...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  const sidebarProps = { links, location, displayName, displayInitial, displayRole, logout };

  return (
    <div className="flex h-screen overflow-hidden bg-background">
      {/* Desktop Sidebar */}
      <aside className="w-[260px] bg-sidebar text-sidebar-foreground hidden md:flex flex-col shrink-0 relative z-20">
        <SidebarContent {...sidebarProps} />
      </aside>

      {/* Main */}
      <main className="flex-1 overflow-y-auto w-full flex flex-col relative z-10">
        <header className="sticky top-0 z-30 bg-background/70 backdrop-blur-xl border-b px-4 md:px-8 py-3 flex items-center justify-between">
